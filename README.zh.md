...
> **GPU 要求**：宿主机需安装 NVIDIA 驱动与 [nvidia-container-toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html)。Windows 需 Docker Desktop + WSL2 + NVIDIA Windows 驱动。`openaudio-s1-mini` 在 ≥8 GB 显存下运行舒适。
...
输出为单声道音频，采样率由解码器决定（`openaudio-s1-mini` 通常为 44.1 kHz）；`pcm` 为裸 s16le 数据。
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `FISHSPEECH_MODEL` | `fishaudio/openaudio-s1-mini` | HuggingFace 仓库 id，或 `/checkpoints/` 下的本地目录名 |
...
- **模型变体**：默认 `fishaudio/openaudio-s1-mini`。`FISHSPEECH_MODEL` 可指向任何 Fish-Speech 兼容的 HuggingFace 仓库（如 `fishaudio/openaudio-s1`），只要包含 LLaMA 权重和 `codec.pth`。
